  ...
  const overview = (
    <>
      <PageTitle
        ...
        description="Qabul suhbati, kategoriyalar, diagnostika va shaxsiy roadmap bitta oqimda."
        action={<button className="portal-primary" onClick={() => setActive("admissions")}><UserCheck size={17} /> Yangi qabul</button>}
      />
      <div className="portal-metrics-grid">
        ...
        <MetricCard icon={CheckCircle2} label="Yakunlangan" value={session.dashboard.completed_attempts || 0} note="Hisobot yaratilgan" tone="green" />
      ...
      <div className="admin-overview-grid">
        <article className="portal-card admin-chart-card">
          <div className="portal-card-head"><div><span>Yangi ish oqimi</span><h2>Qabuldan roadmapgacha</h2></div><Sparkles size={21} /></div>
          <div className="admin-process-list">
            {["Admin suhbat va maqsadlarni kiritadi", "O‘quvchi kategoriyalarga ajratiladi", "Mos diagnostik test tavsiya qilinadi", "Admin bola bilan test o‘tkazadi", "Individual roadmap draft yaratiladi"].map((item, index) => <div key={item}><span>{index + 1}</span><strong>{item}</strong></div>)}
          </div>
        </article>
        <article className="portal-card">
          <div className="portal-card-head"><div><span>Tayyorlik</span><h2>Umumiy holat</h2></div><ShieldCheck size={21} /></div>
          <div className="readiness-donut"><div><strong>{session.dashboard.completed_attempts ? Math.round(((session.dashboard.readiness.find((item) => item.readiness === "ready")?.count ?? 0) / session.dashboard.completed_attempts) * 100) : 0}%</strong><small>Tayyor</small></div></div>
          <div className="donut-legend"><span><i className="ready" />{session.dashboard.readiness.find((item) => item.readiness === "ready")?.count ?? 0} tayyor</span><span><i className="risk" />{session.dashboard.readiness.find((item) => item.readiness === "not_ready")?.count ?? 0} tayyor emas</span></div>
        </article>
      </div>
      <article className="portal-card">
        <div className="portal-card-head"><div><span>Tezkor amallar</span><h2>Keyingi qadamni tanlang</h2></div></div>
        <div className="admin-quick-actions">
          <button onClick={() => setActive("admissions")}><UserCheck size={20} /><strong>Qabul va suhbat</strong><small>Yangi profil yaratish</small></button>
          <button onClick={() => setActive("exams")}><ClipboardList size={20} /><strong>Diagnostika</strong><small>Admin bilan test</small></button>
          <button onClick={() => setActive("categories")}><Target size={20} /><strong>Kategoriyalar</strong><small>Profiling teglarini boshqarish</small></button>
          <button onClick={onOpenReport}><FileBarChart size={20} /><strong>Hisobot</strong><small>Oxirgi diagnostikani ko‘rish</small></button>
        </div>
      </article>
    </>
  );
