...
export async function apiRequest<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const firstFieldError = payload
      ? Object.values(payload).find((value) => Array.isArray(value) && typeof value[0] === "string")
      : null;
    throw new Error(detail ?? (Array.isArray(firstFieldError) ? firstFieldError[0] : null) ?? "Server bilan ishlashda xatolik yuz berdi.");
  }
  return payload as T;
}
